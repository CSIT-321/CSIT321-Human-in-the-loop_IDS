...
| E6 | "Confirm `AL-03086` → show that four *similar* alerts nobody touched moved with it" (plan S16 narrative, v1.1) | **UNPERFORMABLE.** `AL-03086`'s family has one member: the Benign family key includes the destination address. Found by rehearsing the narrative against a database copy. Replaced in plan v1.21 by the `Port Scan / 445` family, where three confirmations move the two unjudged members into the Tier 2 band — asserted by `scripts/rehearse_demo.py` |
