"""Corrected-dataset XGBoost inference with native TreeSHAP explanations.

PROVENANCE
    Vendored from stage-3/core/ml_inference.py (authored by a collaborator) and adapted for the
    corrected CSE-CIC-IDS2018 dataset and our 8-class model. The upstream module targets the
    6-class model trained on the uncorrected dataset and rejects ours outright.

ADAPTATIONS (each marked "ADAPTED:" inline)
    1. Removed the hard 78-feature assertion; the model's own num_features() is authoritative.
    2. Extended FORBIDDEN_PREDICTION_FIELDS with "Attempted Category", "attack_class" and
       "is_attempted" - leakage vectors introduced by the corrected release.
    3. Repointed default artifact paths from stage-3/ to hitl-ids/.

The TreeSHAP logic, additivity verification, schema validation and provenance hashing are the
collaborator's work and are preserved unchanged.
"""
...
# ADAPTED: repointed from stage-3 (6-class, uncorrected) to our corrected 8-class artifacts.
REPO_ROOT = Path(__file__).resolve().parents[4]
HITL_ROOT = REPO_ROOT / "hitl-ids"
STAGE_1_PROCESSED_DIR = HITL_ROOT / "data" / "processed"
DEFAULT_FEATURE_INPUT_PATH = STAGE_1_PROCESSED_DIR / "demo_sample.csv"
MODEL_DIR = HITL_ROOT / "models"
...
# ADAPTED: "Attempted Category" states whether an attack succeeded - a leakage vector
# introduced by the corrected release that the upstream guard predates.
...
def load_model_artifacts(
    ...
    # ADAPTED: the upstream module hard-asserted 78 features (the 6-class model trained on the
    # uncorrected dataset). Our 8-class model uses 82. The model's own num_features() is checked
    # below, which is the authoritative constraint - a fixed literal here is not.
    if not feature_columns:
        raise InferenceConfigurationError("feature-columns.json is empty.")
